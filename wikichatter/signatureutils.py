...
_TIMESTAMP_RE_7a = r"[0-9]{2}:[0-9]{2} [0-9]{1,2} [^\W\d]+ [0-9]{4} \(CEST\)"
# 22:54 mar 2022 (CEST)
_TIMESTAMP_RE_7b = r"[0-9]{2}:[0-9]{2} [^\W\d]+ [0-9]{4} \(CEST\)"
# 01:33 19 may 2007 (CET)
_TIMESTAMP_RE_7c = r"[0-9]{2}:[0-9]{2} [0-9]{1,2} [^\W\d]+ [0-9]{4} \(CET\)"
# 22:54 mar 2022 (CET)
...
_TIMESTAMP_RE_14 = r"[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{4}\/[0-9]{2}:[0-9]{2}\&nbsp\;UTC"
...
# Spanish: [[Usuario:Elvisor|Elvisor]], [[Usuario:Leo3487]], [[Usuaria:<username>|<username>]], [[w:de:User:DaB.|DaB.]]
...
USER_RE = re.compile(r"(\[\[(?:.*:)?\W*(?:user|用户|用戶|使用者|U|Usuario|Usuaria|Benutzer|Benutzerin|Utilisateur|Utilisatrice)\W*:(.*?)(?:\|[^\]]+)?\]\])", re.I)
...
# Arabic: [[مستخدم:Sami Lab|Sami Lab]] ([[نقاش المستخدم:Sami Lab|نقاش]]) 18:41، 24 نوفمبر 2016 (ت ع م)
USER_TALK_RE = re.compile(r"(\[\[(?:.*:)?\W*(?:user[_ ]talk|UT|用户讨论|用戶討論|使用者讨论|使用者討論|Usuario[_ ]Discusión|Usuaria[_ ]Discusión|Benutzer[_ ]Diskussion|Benutzerin[_ ]Diskussion|BD|Discussion[_ ]utilisateur|Discussion[_ ]utilisatrice)\W*:(.*?)(?:\|[^\]]+)?\]\])", re.I)
...
USER_CONTRIBS_RE = re.compile(r"(\[\[(?:.*:)?\W*(?:Special|Especial|Spécial|Spezial):(?:Contributions|用户贡献|用戶貢獻|电邮用户|電郵用戶|Beiträge|Contribuciones|Registro|ListFiles)/(.*?)(?:\|[^\]]+)?\]\])", re.I)
...
def _remove_timestamps_from_wikicode(wcode):
    """
    Removes all timestamps from the wikicode object.

    Args:
        wcode (mwp.wikicode.Wikicode): The parsed wikicode object.

    Returns:
        mwp.wikicode.Wikicode: The wikicode object with timestamps removed.
    """
    nodes = wcode.nodes
    cleaned_nodes = [node for node in nodes if not _node_has_timestamp(node)]
    return mwp.wikicode.Wikicode(cleaned_nodes)


def _remove_signatures_from_wikicode(wcode):
    """
    Removes all signatures from the wikicode object.

    Args:
        wcode (mwp.wikicode.Wikicode): The parsed wikicode object.

    Returns:
        mwp.wikicode.Wikicode: The wikicode object with signatures removed.
    """
    nodes = wcode.nodes
    signature_locations = _find_signatures_in_nodes(nodes)
    cleaned_nodes = [node for idx, node in enumerate(nodes) if not any(start <= idx <= end for start, end in signature_locations)]
    return mwp.wikicode.Wikicode(cleaned_nodes)

def remove_timestamps_and_signatures(text):
    """
    Removes all timestamps and signatures from the provided text.

    Args:
        text (str): The text containing timestamps and signatures to remove.

    Returns:
        str: The text without timestamps and signatures.
    """
    # Parse the text into wikicode
    wcode = mwp.parse(text)

    # Remove timestamps
    wcode = _remove_timestamps_from_wikicode(wcode)

    # Remove signatures
    wcode = _remove_signatures_from_wikicode(wcode)
